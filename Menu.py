...
def searchsubmit():
    ...
    if(search==""):
        ...
        if(Results == []):
            ...
            search_list_box.insert(0,"Book ID | Genre | Title | Author | "
                                     "Purchased Date | Member ID " )
    ...
    if book_id_text == "":
        ...
        if in_library and valid_member:
            ...
            if Available:
                ...
                checkout_error_box["text"] = ("Book Is Not "
    ...
    if return_book_id_text == "":
        ...
        if in_library:
            ...
            if Returned:
                ...
                return_error_box["text"] = ("Book has not been "
                                            "checked out by any member")
                return_error_box.config(bg='red')
        else:
...
def recommendsubmit():
    ...
    if recommend_member_id == "":
        recommend_list_box.insert(0, "Please Enter Member's ID")
    else:
...
title = Label(text="Library Management System", width='600', height='1'
              , fg='white', bg=DarkPurple, font=('Helvetica', 15),
              highlightbackground='black', highlightthickness =2)
title.pack()

# Search Region
search_frame = Frame(window, width='500', height='200', bg=Pink,
                     highlightbackground='black', highlightthickness=2)

search_frame.place(x=16, y=42)

search_title = Label(search_frame, text='Book Search', width='55', height='1',
                     bg=DarkPurple, fg='white', font=('Helvetica', 13))

search_title.place(x=0, y=0)

search_book_title = Label(search_frame, text='Book Title:', font=('calibre', 13)
                          , bg=Pink, fg='white')

search_book_title.place(x=40, y=40)

book_title = StringVar()
search_textbox = Entry(search_frame, font=('calibre', 11), fg='black',
                       textvariable=book_title)

search_textbox.place(x=140, y=40)

search_submit_btn = Button(search_frame, text='Submit', width='10',
                           font=('calibre', 11), fg='white', bg=DarkPurple,
                           command=searchsubmit)

search_submit_btn.place(x=350, y=35)

search_list_box = Listbox(search_frame, width='52', height='5',
                          font=('calibre', 11))
...
# Check Region
checkout_frame = Frame(window, width='500', height='150', bg=Pink,
                       highlightbackground='black', highlightthickness=2)

checkout_frame.place(x=16, y=260)

checkout_title = Label(checkout_frame, text='Book Checkout', width='55',
                       height='1', bg=DarkPurple, fg='white',
                       font=('Helvetica', 13))

checkout_title.place(x=0, y=0)

checkout_book_id = Label(checkout_frame, text='Book ID:',
                         font=('calibre', 13), bg=Pink, fg='white')

checkout_book_id.place(x=40, y=40)

checkout_book_id = Label(checkout_frame, text='Member ID:',
                         font=('calibre', 13), bg=Pink, fg='white')

checkout_book_id.place(x=40, y=70)

book_id_1=StringVar()
checkout_book_id_textbox = Entry(checkout_frame, font=('calibre', 11),
                                 fg='black', textvariable=book_id_1)

checkout_book_id_textbox.place(x=140, y=40)

member_id_1=StringVar()
checkout_member_id_textbox = Entry(checkout_frame, font=('calibre', 11),
                                   fg='black', textvariable=member_id_1)

checkout_member_id_textbox.place(x=140, y=70)

checkout_submit_btn = Button(checkout_frame, text='Submit', width='10',
                             font=('calibre', 11), fg='white', bg=DarkPurple,
                             command=checkoutsubmit)

checkout_submit_btn.place(x=350, y=53)

checkout_error_box = Label(checkout_frame, font=('calibre', 11), bg='white',
                           width='52', height='2')

checkout_error_box.place(x=10, y=100)

#Return Region
return_frame = Frame(window, width='500', height='150', bg=Pink,
                     highlightbackground='black', highlightthickness=2)

return_frame.place(x=16, y=430)

return_title = Label(return_frame, text='Book Return', width='55', height='1',
                     bg=DarkPurple, fg='white', font=('Helvetica', 13))

return_title.place(x=0, y=0)

return_book_id = Label(return_frame, text='Book ID:', font=('calibre', 13),
                       bg=Pink, fg='white')

return_book_id.place(x=40, y=40)

book_id_2=StringVar()
return_book_id_textbox = Entry(return_frame, font=('calibre', 11),
                               fg='black', textvariable=book_id_2)

return_book_id_textbox.place(x=140, y=40)

return_submit_btn = Button(return_frame, text='Submit', width='10',
                           font=('calibre', 11), fg='white', bg=DarkPurple,
                           command=returnsubmit)

return_submit_btn.place(x=350, y=35)

return_error_box = Label(return_frame, font=('calibre', 11), bg='white',
                         width='52', height='2')

return_error_box.place(x=10, y=90)

#Recommend Region
recommend_frame = Frame(window, width='540', height='538', bg=Pink,
                        highlightbackground='black', highlightthickness=2)

recommend_frame.place(x=540, y=42)

recommend_title = Label(recommend_frame, text='Book Recommend', width='59',
                        height='1', bg=DarkPurple, fg='white',
                        font=('Helvetica', 13))

recommend_title.place(x=0, y=0)

recommend_member_id = Label(recommend_frame, text='Member ID:',
                            font=('calibre', 13), bg=Pink, fg='white')

recommend_member_id.place(x=80, y=40)

member_id_3=StringVar()
return_member_id_textbox = Entry(recommend_frame, font=('calibre', 11),
                                 fg='black', textvariable=member_id_3)

return_member_id_textbox.place(x=180, y=40)

recommend_submit_btn = Button(recommend_frame, text='Submit', width='10',
                              font=('calibre', 11), fg='white', bg=DarkPurple,
                              command=recommendsubmit)
...
recommend_error_box = Label(recommend_frame)



#Check For Over 60 days
days_error_frame = Frame(window, width='850', height='97', bg=Pink,
                         highlightbackground='black', highlightthickness=2)

days_error_frame.place(x=16, y=590)

days_error_title = Label(days_error_frame, text='Books Checkout Over 60 Days',
                         width='94', height='1', bg=DarkPurple, fg='white',
                         font=('Helvetica', 13))

days_error_title.place(x=0, y=0)

days_error_label = Label(days_error_frame, font=('calibre', 11),
                         bg='white', width='92', height='3')

days_error_label.place(x=5, y=30)

#Quit Frame
quit_button_frame = Frame(window, width='200', height='77',
                          bg=Pink, highlightbackground='black',
                          highlightthickness=2)

quit_button_frame.place(x=880, y=600)

quit_button = Button(quit_button_frame, text='Quit', width='20', height='3',
                     font=('calibre', 12), fg='white', bg=DarkPurple,
                     command=Quit)
